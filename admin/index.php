...
$sel_ganado = $con->prepare("SELECT SUM(total) AS ganado FROM compras
                            WHERE estado_compra = 'COMPRADO'");
$sel_ganado->execute();
...
$mes = date('Y-m');
$sel_mes = $con->prepare("SELECT SUM(total) AS ganado_mes FROM compras
                            WHERE estado_compra = 'COMPRADO'
                            AND fecha LIKE ?");
...
$hoy = date('Y-m-d');
$sel_hoy = $con->prepare("SELECT SUM(total) AS ganado_hoy FROM compras
                            WHERE estado_compra = 'COMPRADO'
                            AND fecha LIKE ?");
...
//pedidos por enviar
$sel_compras = $con->prepare("SELECT id FROM compras WHERE estado_compra = 'COMPRADO' AND estado_envio = 'POR ENVIAR'");
$sel_compras->execute();
$row_compras = $sel_compras->rowCount();
$sel_compras = null;

//pedidos enviados
$sel_enviados = $con->prepare("SELECT id FROM compras WHERE estado_compra = 'COMPRADO' AND estado_envio = 'ENVIADO'");
$sel_enviados->execute();
$row_enviados = $sel_enviados->rowCount();
$sel_enviados = null;

//pedidos entregados
$sel_entregado = $con->prepare("SELECT id FROM compras WHERE estado_compra = 'COMPRADO' AND estado_envio = 'ENTREGADO'");
...
//Reorden cuando queden menos de 20 productos
...
<div class="container" style="margin-top: 1%;">
    ...
    <div class="row">
        <div class="col">
                <div class="card text-white bg-secondary mb-3" style="max-width: 20rem;">
                        <div class="card-header">Rese&ntilde;as de hoy</div>
                        <div class="card-body">
        ...
        <div class="col">
                <div class="card text-white bg-dark mb-3" style="max-width: 20rem;">
                        ...
                        <div class="card-body">
                        <a href="../reportes/entregados.php"><h1 class="card-title text-center text-white"><?php echo $row_usuarios;?></h1></a>
                        </div>
                </div>
        </div>
    </div>
